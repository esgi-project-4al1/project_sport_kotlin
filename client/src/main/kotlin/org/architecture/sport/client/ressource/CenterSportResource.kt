...
class CenterSportResource(
    ...
    @GetMapping
    fun getCenterSport(
        @RequestParam(required = false) centerSportId: String?,
    ): ResponseEntity<Any> {
        if (centerSportId != null) {
            tryCatchUUID(centerSportId) ?: return ResponseEntity.badRequest().body("Invalid UUID")
        }
        val centerSport = centerSportApi.gets(
            id = centerSportId?.let { UUID.fromString(centerSportId) }
        )
